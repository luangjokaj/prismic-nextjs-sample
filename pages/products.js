...
export default class extends React.Component {
	static async getInitialProps({ req }) {
		try {
			...
			const productsDocument = await Client(request).getSingle(
				"products",
			);
			const productList = await Client(request).query(
		...
		return this.props.productList.map((document, index) => (
			<div key={index} className="products-grid-item-wrapper">
				...
				<p className="products-grid-item-subtitle">
					{RichText.asText(document.data.sub_title)}
				</p>
			</div>
		));
	}
	...
	renderBody() {
		return (
			<Layout
				title={this.props.productsDocument.data.meta_title}
				description={this.props.productsDocument.data.meta_description}
					...
					<div className="l-wrapper">
						<header className="products-grid-header">
							<div className="products-grid-header-title">
								{RichText.render(
									this.props.productsDocument.data.title,
									linkResolver,
								)}
							</div>
						</header>
