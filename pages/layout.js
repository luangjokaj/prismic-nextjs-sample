...
export default class extends React.Component {
			...
			<Head className={this.props.className}>
				<title>
					{this.props && this.props.title
						? RichText.asText(this.props.title)
						: "Not Found"}
				</title>
				<meta
					name="description"
	...
	render() {
		const headerItems = this.props.layout.data.header_nav_items.map(
			(item, index) => (
		...
		const socialItems = this.props.layout.data.footer_social_items.map(
			(item, index) => {
				return (
					<a
						key={index}
		...
		const navItems = this.props.layout.data.footer_nav_items.map(
			(item, index) => (
