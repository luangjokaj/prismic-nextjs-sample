...
export default class extends React.Component {
		...
		return related.map((item, index) => (
			<div key={index} className="products-grid-item-wrapper">
				...
				<p className="products-grid-item-subtitle">
					{RichText.asText(item.product1.data.sub_title)}
				</p>
			</div>
		));
	}
	...
	renderBody() {
		return (
			<Layout
				title={this.props.product.data.meta_title}
				description={this.props.product.data.meta_description}
						...
						<div className="l-wrapper">
							<div className="product-hero-inner">
								<img
									...
									src={
										this.props.product.data.product_image
											.url
									}
									alt={
										this.props.product.data.product_image
					...
					<section className="product-description">
						<div className="l-wrapper">
							...
							<div className="product-description-content">
								{RichText.render(
									this.props.product.data.product_description,
									linkResolver,
								)}
							</div>
						</div>
					...
					<section>
						...
						<div className="products-grid-items-wrapper">
							{this.renderRelatedProducts(
								this.props.product.data.related_products,
							)}
						</div>
					</section>
				</div>
		...
		if (this.props.error)
			return (
				<Layout layout={this.props.layout}>
					<NotFound msg="this product doesn't exists." />
				</Layout>
			);
