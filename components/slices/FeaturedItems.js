	...
	return slice.items.map((item, index) => (
		<div key={index} className="products-grid-item-wrapper">
			<img
				...
				src={item.link_to_product.data.product_image.url}
				alt={item.link_to_product.data.product_image.alt}
			/>
			<p className="products-grid-item-name">
				<PrismicLink to={item.link_to_product}>
					<a>
						{RichText.asText(
							item.link_to_product.data.product_name,
						)}
					</a>
				</PrismicLink>
			</p>
			<p className="products-grid-item-subtitle">
				{RichText.asText(item.link_to_product.data.sub_title)}
			</p>
		</div>
	));
}
